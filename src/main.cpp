...
// Copyright (c) 2013 32 developers
...
const string strMessageMagic = "32 Signed Message:\n";
...
static const int64 nStartSubsidy = 0.0032 * COIN;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("32Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("32Miner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running 32Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
